...
# vmas 환경을 다루기 위한 wrapper 클래스
# 내부적으로 vmas의 환경 객체를 받아 다양한 기능을 제공하여 강화학습 프레임워크에서 쉽게 사용할 수 있도록 도움
class VmasWrapper(_EnvWrapper):                                 # 아... 또 상속받고 있네.. 어우 머리야
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(**kwargs, allow_done_after_reset=True)         # 또 추가 인자들을 상위 클래스에 전달...

    # 아래의 vmas env에서 넘어왔지만 딱히 하는 일은 없어
    def _build_env(
        self,
...
class VmasEnv(VmasWrapper):
    ...
    def __init__(
        ...
        **kwargs,       # 추가적인 시나리오 인자 전달 가능
        ...
        # 상위 클래스인 vmaswrapper의 생성자를 호출하면서 인자를 넘김
        # 그럼 vmaswrapper에서 이러한 인자들로 뭘 하는거여
        super().__init__(
            scenario=scenario,
            num_envs=num_envs,
    ...
    def _build_env(
        ...
        seed: Optional[int],
        **scenario_kwargs,      # 시나리오 별로 추가적인 설정값들
    ) -> "vmas.simulator.environment.environment.Environment":  # noqa, build)env가 이 화살표가 가리키는 타입의 객체를 반환한다는 소리
        vmas = self.lib
        ...
        return super()._build_env(              # 결국 상위 클래스에서 환경 생성
            env=vmas.make_env(
                scenario=scenario,
                num_envs=num_envs,
                device=self.device
