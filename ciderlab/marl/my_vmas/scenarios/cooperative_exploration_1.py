        ...
        if is_last:
            ...
            for i, target in enumerate(self._targets):
                ...
                if not mask.any():
                    ...
                    new_cost_targets.append(self.cost_targets[:, i])
                    new_finish_cost_targets.append(self.finish_cost_targets[:, i])
            # 타겟 리스트와 관련 텐서들을 업데이트
            self._targets = new_targets
            if new_cost_targets:
                self.cost_targets = torch.stack(new_cost_targets, dim=1)
                self.finish_cost_targets = torch.stack(new_finish_cost_targets, dim=1)
            else:
                self.cost_targets = torch.zeros(self.world.batch_dim, 0, device=self.world.device)
                self.finish_cost_targets = torch.zeros(self.world.batch_dim, 0, device=self.world.device)
            self.n_targets = len(new_targets)
