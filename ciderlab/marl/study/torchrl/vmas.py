...
class VmasEnv(VmasWrapper):
    ...
    def __init__(
        ...
        # 시뮬레이션할 vmas 시나리오의 이름 또는 사용자 정의 시나리오 객체. 반드시 사용 가능한 시나리오일 것? 사용 가능이 뭔데
        *,
        num_envs: int,
    ...
    def _build_env(
        ...
        **scenario_kwargs,
        ...
        # vmas로부터 생성된 환경 객체를 vmaswrapper 로 래핑하여, torchrl의 인터페이스로 동작할 수 있도록 함
        return super()._build_env(
            # 사용자가 전달한 시나리오 이름 또는 시나리오 객체에 맞게 vmas 환경을 초기화
            env=vmas.make_env(
                scenario=scenario,
                num_envs=num_envs,
                device=self.device
                ...
                seed=seed,
                wrapper=None,
                # 시나리오 별로 필요한 추가 인자들을 전달. 예를 들어 에이전트의 수, 보상 정책 등
                **scenario_kwargs,
            ),
    ...
    # 객체의 문자열 표현을 반환
    # 시나리오 이름을 포함하여 환경의 상태를 직관적으로 확인 가능
    def __repr__(self):
