...
class Scenario(BaseScenario):
    ...
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        for i in range(self.n_agents):
            ...
            agent.target_idx = agent.pos_rew.clone()
        ...
        self.agents_path = self.pos_rew.clone()
    ...
    def reset_world_at(self, env_index: int = None):
        ...
        # 목표 생성해서 에이전트한테 할당
        # 근데 랜덤 엔터티 생성에 랜덤 위치 할당하는 코드가 포함되어 있는데
        # 왜 여기서 목표의 위치를 또 찾아서 이 난리를 치는 거지?
        ...
        # 무난하게 시나리오랑 같이 넣어둡시다
        if env_index is not None:
            self.agents_path[env_index] = random_choice(self.world.agents, self.targets)
    ...
    def reward(self, agent: Agent):
        ...
        #         # 한 에이전트는 4개의 차원에서 병렬로 행동 중
        #         # 그 중에 목표에 도달한 에이전트가 있다면
        #         if agent.on_goal.any():
        #             # 해당 차원의 에이전트 목표 인덱스를 1 증가
        #             if agent.target_idx[agent.on_goal].item() < 3:
        #                 # 다음 목표
        #                 agent.target_idx[agent.on_goal] += 1
        #                 agent.goal[agent.on_goal] = self.agents_path[agent.target_idx.item()]
        #             elif agent.target_idx[agent.on_goal].item() == 3:
        #                 agent.target_idx[agent.on_goal] += 1
        #                 agent.goal[agent.on_goal] = self.finished_targets[agent_idx]
                        
        #             else:
        #                 continue
                    
        #             agent.pos_shaping[agent.on_goal] = (
        #                         torch.linalg.vector_norm(
        #                             agent.state.pos - agent.goal.state.pos,
        #                             dim=1,
        #                         )
        ...
        if is_last:
            for agent_idx, agent in enumerate(self.world.agents):
                # 현재 배치 차원에서 on_goal이 True인 인덱스들 구하기
                mask = agent.on_goal.clone()  # 변경 전 마스크 복사
                if mask.any():
                    true_indices = mask.nonzero(as_tuple=False).squeeze(-1)
                    for d in true_indices:
                        if agent.target_idx[d] < 3:
                            # 다음 목표: 현재 에이전트의 경로에서 해당 target_idx를 이용
                            agent.target_idx[d] += 1
                            agent.goal[d] = self.agents_path[d][agent_idx][int(agent.target_idx[d].item())]
                        elif agent.target_idx[d] == 3:
                            agent.target_idx[d] += 1
                            agent.goal[d] = self.finished_targets[agent_idx]
                        else:
                            continue

                    # 목표 갱신 후 해당 인덱스에 대해 pos_shaping 업데이트
                    # 계산 시, 각 차원별 위치 차이가 반영되도록 mask를 사용합니다.
                    new_distance = torch.linalg.vector_norm(
                        agent.state.pos - agent.goal.state.pos,
                        dim=1,
                    )
                    agent.pos_shaping[mask] = new_distance[mask] * self.pos_shaping_factor

                    # 업데이트가 완료되었으므로 해당 마스크를 초기화
                    agent.on_goal[mask] = False
